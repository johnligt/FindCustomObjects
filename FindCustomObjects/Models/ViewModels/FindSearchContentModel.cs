...
namespace FindCustomObjects.Models.ViewModels
{
    public class FindSearchContentModel : PageViewModel<FindSearchPage>
    {
        ...
        /// </summary>
        public int PagingSection => 1 + (PagingPage - 1) / PagingSectionSize;
        ...
        public int PagingSectionSize => 10;

        /// <summary>
        /// Calculate the number of paging sections required to list page links
        /// </summary>
        public int TotalPagingSections => 1 + (TotalPagingPages - 1) / PagingSectionSize;

        /// <summary>
        /// Number of first page in current paging section
        /// </summary>
        public int PagingSectionFirstPage => 1 + (PagingSection - 1) * PagingSectionSize;

        /// <summary>
        /// Number of last page in current paging section
        /// </summary>
        public int PagingSectionLastPage => Math.Min((PagingSection * PagingSectionSize), TotalPagingPages);
        ...
        /// </summary>
        public string Query => (HttpContext.Current.Request.QueryString["q"] ?? string.Empty).Trim();

        /// <summary>
        /// Search tags like language and site
        /// </summary>
        public string Tags => string.Join(",", ServiceLocator.Current.GetInstance<IStatisticTagsHelper>().GetTags());

        /// <summary>
        /// Length of excerpt
        /// </summary>
        public int ExcerptLength => CurrentPage.ExcerptLength;

        /// <summary>
        /// Height of hit images
        /// </summary>
        public int HitImagesHeight => CurrentPage.HitImagesHeight;

        /// <summary>
        /// Flag retrieved from editor settings to determine if it should 
        /// use AND as the operator for multiple search terms
        /// </summary>
        public bool UseAndForMultipleSearchTerms => CurrentPage.UseAndForMultipleSearchTerms;
